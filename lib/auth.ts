...
export const checkAuth = async (callback: Function, isGetMethod = true) => {
	...
	if (session && session.user) {
		// Upsert: create the user row with safe defaults if it doesn't exist yet.
		// This handles the case where Supabase Auth created the user but the
		// Prisma users table row was never inserted (e.g. first magic link login).
		let user = await prisma.users.findUnique({ where: { id: session.user.id } });
		...
		// If still null after upsert attempt, return 401 instead of crashing
		if (!user) {
